...
int main() {
    ...
    switch (drawing) {
        ...
        case 2:{
            Object kvet;
            kvet.position = (Vector2){4, 4};
            kvet.size = (Vector2){8,8};
...
void vykreslitKvetiny(Object objects[], int count){
    for (int i = 0; i < 1; i++)
    {
        ...
        move_to(curObj.position.y + curObj.size.y/4-curObj.size.y/8, curObj.position.x*2 + curObj.size.x/2-curObj.size.x/4+1);
        set_yellow_color();        
        for (int i = 0; i < curObj.size.y/2+1; i++)
        {
            for (int j = 0; j < curObj.size.x/2 + 1; j++)
            {
                pixel();
            }
            move_to(curObj.position.y + curObj.size.y/2-curObj.size.y/4 + i, curObj.position.x*2+curObj.size.x/4+1);
        }
